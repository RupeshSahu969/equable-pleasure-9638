...
export default function LoginPage() {
  ...
  const handleSubmitGoogle = () => {
    ...
    signInWithPopup(isAuth, providers)
      .then((result) => {
        ...
        dispatch(authFun(!auth, result._tokenResponse.firstName, result._tokenResponse.email))
        navigate("/tracker")
      })
      .catch((error) => {
        console.log(error);
  ...
  return (
    <Stack bg={"#f2f6f8"} minH={'100vh'} direction={{ base: 'column', md: 'row' }}>
      ...
      <Flex p={5} flex={1} align={'center'} justify={'center'} >
        ...
        <Stack spacing={6} w={'full'} maxW={'full'}>
          ...
          <Flex gap={20} justify={'space-between'}>
            ...
            <Stack>
              <Text color={'#999'} fontSize={'1rem'} >Don't have an account? <Link color={'#03a9f4'} _hover={'textDecoration:none '} as={ReachLink} to='/signup'> Sign up </Link> </Text>
            </Stack>
          </Flex>
          ...
          >
            ...
            <Stack spacing={5} mx={'auto'} w={'380px'} maxW={'auto'} py={12} px={6}>
                ...
                <form action="" onSubmit={handleLogin}>
                  <Stack spacing={4}>
                    ...
                    <FormControl id="password" isRequired>
                      {/* sd */}
                      <InputGroup>
                        <Input value={formPassword} onChange={(e) => setformPassword(e.target.value)} placeholder='Enter password' type={showPassword ? 'text' : 'password'} />
                    ...
                    <Stack spacing={10}>
                      <Stack
                        ...
                        <Link fontWeight={500} fontSize={'14px'} color={'#03a9f4'}>Forgot password?</Link>
                      </Stack>
                    </Stack>
                    <Stack spacing={10} pt={2}>
                      <Input disabled={formEmail === "" || formPassword === ""} cursor={'pointer'} _hover={{
                        bg: 'blue.500',
                      }} color={'#fff'} borderRadius={'2px'} bg={'#03a9f4'} value={'LOG IN'} type='submit' />
                    </Stack>
                  </Stack>
                </form>
      ...
      <Flex pt={10} flex={0.58} bg={'linear-gradient(180deg,#673ab7,#3f51b5,#00bcd4,#3f51b5)'} >
        {/* <Center> */}
        <Stack textAlign={'center'} >
          <Center>
            <div id={styles.newText} >
              NEW
            </div>
          </Center>
          <div id={styles.Text} >
          ...
          <div id={styles.para} >
            Visualize projects on a timeline and plan team's <br /> capacity (see who's busy and who's available).
          </div>
          <Center>
